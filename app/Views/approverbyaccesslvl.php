			...
			<div class="content-wrapper">
				...
				<!-- Default box -->
                    ...
                    <div class="box box-primary">
                        ...
                        <h3 class="box-title">Approver By Access Level</h3>
                        </div>
                        <div class="box-body">
                            <?php
                                if (isset($get->apply_accesslvl_id)) {
                                    echo form_open("utilities/update_approver_by_access_level?id=".$_GET['id']);
                                }else{
                                    echo form_open("utilities/submit_approver_by_access_level");
                                }
                            ?>
                                <div class="form-group">
									...
									<?php
										foreach ($accesslvls as $val) {
                                            $check = '';
                                            if (isset($get->apply_accesslvl_id)) {
                                                if ($get->apply_accesslvl_id == $val->id) {
                                                    $check = 'selected';
                                                }
                                            }
                                ...
                                <div class="form-group">
									...
									<?php
										foreach ($accesslvls as $val) {
                                            $check = '';
                                            if (isset($get->lvl1_approve_accesslvl_id)) {
                                                if ($get->lvl1_approve_accesslvl_id == $val->id) {
                                                    $check = 'selected';
                                                }
                                            }
											echo '<option value="'.$val->id.'" '.$check.'>'.$val->access_name.'</option>';
                                ...
                                <div class="form-group">
									...
									<?php
										foreach ($accesslvls as $val) {
                                            $check = '';
                                            if (isset($get->lvl2_approve_accesslvl_id)) {
                                                if ($get->lvl2_approve_accesslvl_id == $val->id) {
                                                    $check = 'selected';
                                                }
                                            }
											echo '<option value="'.$val->id.'" '.$check.'>'.$val->access_name.'</option>';
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a type="button" href="<?=site_url('utilities/approver_by_access_level?id='.$val->encode_id)?>" class="btn btn-info"><i class="fa fa-edit"></i></a>
                                                <button type="button" value="<?=site_url('utilities/del_approver_by_access_level/'.$val->approver_id)?>" class="btn btn-danger del-menu"><i class="fa fa-remove"></i></button>
                                            </td>
                                        </tr>
                                    <?php
                                        $count++;
